{
  ...
  outputs =
    {
      ...
      home-manager,
      ...
    }@inputs: inputs.flake-parts.lib.mkFlake {inherit inputs;} (top@{config, withSystem, moduleWithSystem,...}:
    {
      imports = [
  inputs.devshell.flakeModule
      ...
      perSystem = { config, pkgs, ... }: {
        ...
        devshells = {
          default ={
            env = [{name = "TEST_VARIABLE"; value = "works";}];
            commands = [ { command = "cowsay hello | lolcat"; name ="hello"; }];
            packages = with pkgs; [ lolcat cowsay];
          };
        };
      };
