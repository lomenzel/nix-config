            ...
            then {
              firefox-unwrapped = prev.firefox-unwrapped.overrideAttrs (old: {
                postPatch =
                  (old.postPatch or "")
                  + ''
                    # Replace 'nullptr' with '0' for Vulkan image and memory handles to fix 32-bit compilation
                    sed -i -E 's/(mNv12Image|mNv12Mem)\[([^]]+)\] = nullptr;/\1[\2] = 0;/g' dom/media/platforms/ffmpeg/FFmpegVulkanVideoDecoder.cpp
                  '';
              });
              ...
              buildMozillaMach = opts:
                final.callPackage
                (import "${inputs.mozilla-fix}/pkgs/build-support/build-mozilla-mach/default.nix" opts)
                {};
            }
            else {
              gnome-chess = prev.gnome-chess.overrideAttrs (old: {
    ...
    packages =
      builtins.mapAttrs (
        ...
        in {
          ...
          tuta = pkgs.callPackage ./packages/tuta.nix {};
        }
      )
      supportedSystems;
